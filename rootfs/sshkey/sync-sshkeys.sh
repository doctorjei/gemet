#!/usr/bin/env bash
#
# sync-sshkeys.sh — Merge /etc/yggdrasil/authorized_keys into
# /root/.ssh/authorized_keys, idempotently and non-destructively.
#
# Invocation: yggdrasil-sshkey-sync.service (systemd oneshot), once per boot.
...
SOURCE="${YGGDRASIL_SSHKEY_SOURCE:-/etc/yggdrasil/authorized_keys}"
TARGET="${YGGDRASIL_SSHKEY_TARGET:-/root/.ssh/authorized_keys}"
